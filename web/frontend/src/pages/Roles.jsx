...
const Roles = () => {
  ...
  const isSuperAdmin = (user?.role || '').toLowerCase() === SUPER_ADMIN_ROLE_NAME;
  ...
  // Le rôle SUPER_ADMIN ne doit jamais être exposé à un tenant : on le masque
  // côté UI (défense en profondeur) même si l'API le renvoyait. La source de
  // vérité reste le rôle réel stocké dans AuthContext/JWT.
  const visibleRoles = useMemo(() => {
    if (isSuperAdmin) return roles;
    return roles.filter(r => (r.name || '').toLowerCase() !== SUPER_ADMIN_ROLE_NAME);
  }, [roles, isSuperAdmin]);

  const visiblePresets = useMemo(() => {
    if (isSuperAdmin) return presets;
    return presets.filter(p => (p.name || '').toLowerCase() !== SUPER_ADMIN_ROLE_NAME);
  }, [presets, isSuperAdmin]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (status === 403) {
        // Refus de permission : pas de notification, l'action était
        // de toute façon désactivée pour ce rôle.
        return;
      }
      if (status === 409) {
