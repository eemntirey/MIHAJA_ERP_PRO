...
// Client API unique pour web et desktop.
// Inclut tous les services REST et l'intercepteur de synchronisation hors-ligne.
...
// ======================================================
// MUTATION TRACKER POUR SYNCHRONISATION HORS-LIGNE
// ======================================================
...
api.interceptors.request.use(
    (config) => {
        if (shouldQueue(config)) {
            config.headers = config.headers || {};
            config.headers['X-Offline-Queued'] = 'true';
            config._queued = true;
        }
        return config;
    },
...
export const livreurService = {
    ...
    getMoi: () => api.get('/livreurs/moi'),
    getMoiLivraisons: (params) => api.get('/livreurs/moi/livraisons', { params }),
    getMoiLivraison: (id) => api.get(`/livreurs/moi/livraisons/${id}`),
    addSuiviToMoi: (id, data) => api.post(`/livreurs/moi/livraisons/${id}/suivi`, data),
    passerAuStatutToMoi: (id, data) => api.post(`/livreurs/moi/livraisons/${id}/statut`, data),
    associerUtilisateur: (id, utilisateur_id) => api.post(`/livreurs/${id}/associer-utilisateur`, { utilisateur_id }),
};
...
export const stagiaireService = {
    getAll: (params) => api.get('/stagiaires', { params }),
...
// ======================================================
// DESK SYNC SERVICES (favoris, colonnes, filtres)
// ======================================================

export const favoriteService = {
    getAll: () =>
        api.get('/desk/favorites'),

    add: (item) =>
        api.post('/desk/favorites', item),

    remove: (id) =>
        api.delete(`/desk/favorites/${encodeURIComponent(id)}`),
};

export const columnConfigService = {
    get: (module) =>
        api.get(`/desk/columns/${encodeURIComponent(module)}`),

    save: (module, config = {}) =>
        api.post(`/desk/columns/${encodeURIComponent(module)}`, config),

    reset: (module) =>
        api.delete(`/desk/columns/${encodeURIComponent(module)}`),
};

export const filterPresetService = {
    getAll: (module) =>
        api.get(`/desk/filters/${encodeURIComponent(module)}`),

    save: (module, preset = {}) =>
        api.post(`/desk/filters/${encodeURIComponent(module)}`, preset),

    delete: (module, id) =>
        api.delete(`/desk/filters/${encodeURIComponent(module)}/${encodeURIComponent(id)}`),
};

// ======================================================
// SYNC SERVICES (push/pull hors-ligne)
// ======================================================

export const syncService = {
    push: (mutations) =>
        api.post('/desk/sync/push', { mutations }),

    pull: () =>
        api.get('/desk/sync/pull'),

    status: () =>
        api.get('/desk/sync/status'),

    events: (since) =>
        api.get('/desk/events', { params: since ? { since } : undefined }),
};
